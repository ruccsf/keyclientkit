...
如果采集时程序以 exit code 4 退出并打印以下错误，说明企查查积分余额不足，**整个工作流终止**：
```
❌ 企查查积分余额不足，数据采集无法继续。
   积分不足: shareholders, personnel, finance...（X/8 个工具）
   建议: 充值企查查积分或使用会员账号后重新运行
```
此时告知用户积分不足，停止报告生成。不要尝试用 Web Search 替代 QCC 数据。
